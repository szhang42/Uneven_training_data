...
def train(args, train_dataset, model, tokenizer, flag_in_training):
    ...
    for epoch, _ in enumerate(train_iterator):
        ...
        # train_result = compute_metrics(args.task_name, train_logits, train_golds)

        train_result = compute_metrics(args.task_name, np.argmax(train_logits, axis=1), train_golds)

        # train_result = dis_compute_metrics(args.task_name, train_logits, train_golds)
...
def lp_finetune_load_and_cache_examples(args, task, tokenizer, label_flag, evaluate=False, data_split="finetune"):
    if args.local_rank not in [-1, 0] and not evaluate:
        # Make sure only the first process in distributed training process the dataset,
        # and the others will use the cache
        torch.distributed.barrier()
...
def main():
    ...
    parser.add_argument('--save_steps', type=int, help="Save steps for checkpoints")
