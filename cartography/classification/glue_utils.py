...
def adapted_glue_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        original_id.append(example.guid)
        if ex_index < 5:
            logger.info("*** Example ***")
...
def adapted_glue_compute_metrics(task_name, preds, labels):
    ...
    try:
      ...
      if task_name in ["snli", "winogrande", "toxic"]:
        ...
        final_label = np.argmax(np.array(labels), -1).tolist()
        print(len(preds), len(final_label))
        return glue_compute_metrics("mnli", preds, final_label)
