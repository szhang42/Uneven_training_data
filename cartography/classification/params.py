...
class Params:
  ...
  def __init__(self, MODEL_CLASSES, ALL_MODELS, processors, configs):
      ...
      self.finetune: str = configs.get("finetune", None)
      self.unlabel: str = configs.get("unlabel", None)
      ...
      #when train from the checkpoint (and the checkpoint is a path), remove the below line code
      ...
      # Whether to finetune.
      self.do_finetune: bool = configs.get("do_finetune", False)

      # Whether to label propagation.
      self.label_propagation: bool = configs.get("label_propagation", False)

      # Whether to temperature.
      self.do_temperature: bool = configs.get("do_temperature", False)

      # Whether to train_label_smooth.
      self.do_train_label_smooth: bool = configs.get("do_train_label_smooth", False)
      ...
      self.evaluate_during_training : bool = configs.get("evaluate_during_training", False)
      ...
      # The initial finetune learning rate for Adam.
      self.finetune_learning_rate: float = configs.get("finetune_learning_rate", 1e-5)
      ...
      # Total number of finetuning epochs to perform.
      self.ft_num_train_epochs: float = configs.get("ft_num_train_epochs", 300.0)
      ...
      # Use the existing eval data set
      self.use_existing_eval_data: bool = configs.get("use_existing_eval_data", False)


      # Use different setting
      self.setting: str = configs.get("setting", "01")
